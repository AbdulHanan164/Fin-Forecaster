...
export const getAnnouncements = async (symbol) => {
    const cacheKey = `announcements_${symbol}`;
    if (apiCache[cacheKey]) return apiCache[cacheKey];
    const response = await axios.get(`${API_BASE_URL}/company/${symbol}/announcements`);
    apiCache[cacheKey] = response.data;
    return response.data;
};
