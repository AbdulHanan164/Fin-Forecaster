...
const SECTOR_COLORS = [
    ...
    '#c026d3', '#ca8a04', '#059669', '#4f46e5', '#e11d48',
    '#6d28d9', '#15803d', '#b45309', '#0e7490', '#9333ea',
];
    ...
    return (
        <div
            ...
            style={{
                ...
                boxShadow: isSelected ? `0 4px 16px ${accentColor}20` : '0 1px 4px rgba(0,0,0,0.06)',
            }}
        >
            {/* Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <div style={{ fontWeight: 800, fontSize: 13, color: isSelected ? accentColor : '#1e293b', lineHeight: 1.3, maxWidth: '70%' }}>
                    {sector}
                </div>
                <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
                <div>
                    <div style={{ fontSize: 10, color: '#94a3b8' }}>Volume</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#475569' }}>{fmtVol(totalVol)}</div>
                </div>
                <div>
                    <div style={{ fontSize: 10, color: '#94a3b8' }}>Adv / Dec</div>
                ...
                {topGainer && (
                    <div>
                        <div style={{ fontSize: 10, color: '#94a3b8' }}>Top Gainer</div>
                ...
                {topLoser && (
                    <div>
                        <div style={{ fontSize: 10, color: '#94a3b8' }}>Top Loser</div>
            ...
            <div style={{ display: 'flex', height: 4, borderRadius: 4, overflow: 'hidden', background: '#f1f5f9' }}>
        ...
        <div style={{
            ...
            <div style={{ padding: '14px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
                <div style={{ fontWeight: 800, fontSize: 14, color: '#0f172a' }}>
                    {sector} — <span style={{ color: '#94a3b8', fontWeight: 500, fontSize: 13 }}>{filtered.length} companies</span>
                </div>
                <input
                    ...
                    style={{
                        ...
                        borderRadius: 8, padding: '7px 14px', color: '#0f172a', fontSize: 13,
                    ...
                    <tbody>
                        {filtered.map((row, i) => {
                            ...
                            return (
                                ...
                                >
                                    <td style={{ padding: '10px 14px', fontWeight: 700, color: '#2563eb', whiteSpace: 'nowrap' }}>{row.symbol}</td>
                                    <td style={{ padding: '10px 14px', color: '#475569', maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.company_name}</td>
                                    <td style={{ padding: '10px 14px', textAlign: 'right', color: '#0f172a' }}>{fmt(row.ldcp)}</td>
                                    <td style={{ padding: '10px 14px', textAlign: 'right', color: '#64748b' }}>{fmt(row.open)}</td>
                                    <td style={{ padding: '10px 14px', textAlign: 'right', color: '#64748b' }}>{fmt(row.high)}</td>
                                    <td style={{ padding: '10px 14px', textAlign: 'right', color: '#64748b' }}>{fmt(row.low)}</td>
                                    <td style={{ padding: '10px 14px', textAlign: 'right', color: '#0f172a', fontWeight: 600 }}>{fmt(row.current)}</td>
                                    <td style={{ padding: '10px 14px', textAlign: 'right', color: retColor, fontWeight: 600 }}>
                                        {row.change !== null ? (row.change >= 0 ? '+' : '') + fmt(row.change) : '—'}
                                    </td>
                                    <td style={{ padding: '10px 14px', textAlign: 'right', color: '#64748b' }}>{fmtVol(row.volume)}</td>
                        ...
                        {filtered.length === 0 && (
                            <tr>
                                <td colSpan={COLS.length} style={{ padding: 32, textAlign: 'center', color: '#94a3b8' }}>
...
const AnalysisCard = ({ title, children, fullWidth = false }) => (
    <div style={{
        ...
        <div style={{ fontWeight: 800, fontSize: 13, color: '#1e293b', marginBottom: 16, textTransform: 'uppercase', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: 8 }}>
    ...
    return (
        <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ position: 'absolute', textAlign: 'center' }}>
                <div style={{ fontSize: 24, fontWeight: 900, color: '#0f172a' }}>{data.total}</div>
                <div style={{ fontSize: 10, color: '#94a3b8', textTransform: 'uppercase' }}>Stocks</div>
            </div>
        </div>
    );
};
                    ...
                    {sectorStats.map(s => {
                        ...
                        return (
                            <div key={s.sector} style={{
                                ...
                                <div style={{ fontSize: 16, fontWeight: 900, color: s.avg >= 0 ? '#15803d' : '#b91c1c' }}>
                                    {s.avg >= 0 ? '+' : ''}{s.avg}%
                                </div>
                            </div>
                        );
            ...
            <AnalysisCard title="Sector Leaders" fullWidth>
                <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        <thead>
                            <tr style={{ background: '#f8fafc' }}>
                                <th style={{ padding: '10px 14px', textAlign: 'left', color: '#64748b', fontSize: 11, textTransform: 'uppercase', borderBottom: '1px solid #e2e8f0' }}>Sector</th>
                                <th style={{ padding: '10px 14px', textAlign: 'left', color: '#64748b', fontSize: 11, textTransform: 'uppercase', borderBottom: '1px solid #e2e8f0' }}>Leading Stock</th>
                                <th style={{ padding: '10px 14px', textAlign: 'right', color: '#64748b', fontSize: 11, textTransform: 'uppercase', borderBottom: '1px solid #e2e8f0' }}>Return %</th>
                                <th style={{ padding: '10px 14px', textAlign: 'right', color: '#64748b', fontSize: 11, textTransform: 'uppercase', borderBottom: '1px solid #e2e8f0' }}>Volume</th>
                            </tr>
                        </thead>
                        <tbody>
                            {sectorStats.slice(0, 15).map((s, i) => (
                                <tr key={s.sector} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                    <td style={{ padding: '8px 14px', color: '#475569' }}>{s.sector}</td>
                                    <td style={{ padding: '8px 14px', color: '#2563eb', fontWeight: 700 }}>{s.leader?.symbol}</td>
                                    <td style={{ padding: '8px 14px', textAlign: 'right', color: '#16a34a', fontWeight: 700 }}>+{s.leader?.return?.toFixed(2)}%</td>
...
const IndustryOverview = () => {
    ...
    if (loading) return (
        <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ textAlign: 'center' }}>
                <div style={{ width: 40, height: 40, border: '3px solid #dbeafe', borderTopColor: '#2563eb', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 14px' }} />
                <div style={{ color: '#94a3b8', fontSize: 14 }}>Loading market summary...</div>
    ...
    if (error) return (
        <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
            <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: 32, textAlign: 'center', maxWidth: 500 }}>
                <div style={{ color: '#dc2626', fontWeight: 700, fontSize: 16, marginBottom: 8 }}>Failed to load market data</div>
                <div style={{ color: '#64748b', fontSize: 13, marginBottom: 20 }}>{error}</div>
                <button
                    onClick={() => { setLoading(true); fetchData(true); }}
                    style={{ background: '#2563eb', color: 'white', border: 'none', borderRadius: 8, padding: '10px 24px', fontWeight: 700, cursor: 'pointer' }}
                >
                    Retry
                </button>
            </div>
        ...
        <div style={{ minHeight: '100vh', background: '#f8fafc', color: '#0f172a', fontFamily: "'Inter', sans-serif" }}>
            <div style={{ maxWidth: 1440, margin: '0 auto', padding: '24px 24px 48px' }}>
                ...
                <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                    <div>
                        <h1 style={{ fontSize: 26, fontWeight: 900, margin: 0, color: '#0f172a' }}>
                            Industry Overview
                        </h1>
                    ...
                    <button
                        ...
                        style={{
                            background: '#ffffff',
                            border: '1px solid #e2e8f0',
                            borderRadius: 8, padding: '8px 18px', color: '#2563eb',
                ...
                {overallStats && (
                    <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 24 }}>
                        <StatCard label="Sectors" value={visibleSectors.length} sub={`of ${allSectors.length} total`} color="#2563eb" />
                        <StatCard label="Companies" value={overallStats.totalCompanies} color="#7c3aed" />
                        <StatCard label="Avg Return" value={`${overallStats.avgReturn >= 0 ? '+' : ''}${overallStats.avgReturn.toFixed(2)}%`} color={colorForReturn(overallStats.avgReturn)} />
                        <StatCard label="Advancing" value={overallStats.advancing} color="#16a34a" />
                        <StatCard label="Declining" value={overallStats.declining} color="#dc2626" />
                        <StatCard label="Total Volume" value={fmtVol(overallStats.totalVolume)} color="#d97706" />
                    </div>
                )}
                ...
                <div style={{
                    ...
                    <div style={{ fontSize: 11, color: '#94a3b8', fontWeight: 700, letterSpacing: '0.08em', marginBottom: 10, textTransform: 'uppercase' }}>
                        Filter by Sector
                    </div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                        <button
                            ...
                            style={{
                                ...
                                border: `1.5px solid ${selectedSectors.length === 0 ? '#2563eb' : '#e2e8f0'}`,
                                background: selectedSectors.length === 0 ? '#dbeafe' : '#f8fafc',
                                color: selectedSectors.length === 0 ? '#1d4ed8' : '#64748b',
